...
Crawler.prototype._crawlTick = function () {
  ...
  urlList.getNextUrl().then(function (urlObj) {
    ...
    return self._processUrl(url).catch(error.RobotsNotAllowedError, function () {
      return urlList.insert(new Url({
        url: url,
...
Crawler.prototype._getOrDownloadRobots = function (url) {
  ...
  // server to get it.
  return this._downloadUrl(robotsUrl).then(function (response) {
    var body;
