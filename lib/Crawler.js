...
Crawler = function (opts) {
  ...
  this._cookies = typeof opts.cookies !== "undefined" ? opts.cookies : true;
  this._handlers = [];
};
...
/**
 * Get the cookies option. True if cookies are enabled, false if they are
 * disabled.
 *
 * @return {Boolean} True if enabled.
 */
Crawler.prototype.getCookies = function () {
  return Boolean(this._cookies);
};
...
Crawler.prototype._processUrl = function (url) {
  ...
  }).catch(error.HandlersError, function () {
    return new Url({
      url: url,
